 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */function M(r,e,t){return r?e(r):t==null?void 0:t(r)}class Fe extends f{constructor(){super();c(this,"_handleClick",()=>(this._clicked=!0,this.dispatchEvent(new CustomEvent("panel-cell:clicked"))));this._clicked=!1}static get properties(){return{animationTime:{type:Number},_clicked:{type:Boolean,state:!0}}}updated(t){super.updated(t),t.has("animationTime")&&this.style.setProperty("--mole-animation-duration",`${this.animationTime}ms`)}connectedCallback(){super.connectedCallback(),this.addEventListener("click",this._handleClick)}disconnectedCallback(){super.disconnectedCallback(),this.removeEventListener("click",this._handleClick)}}const Ge=E`
  :host {
    display: flex;
...
`;class ge extends Je{render(){return d` <div class="score">${this._points}</div>`}}c(ge,"styles",[Ke]);window.customElements.define("score-panel",ge);class Ze extends f{constructor(){super();c(this,"handleLogin",()=>this.dispatchEvent(new CustomEvent("login-panel:login",{detail:this.user})))}static get properties(){return{user:{type:String}}}updated(t){super.updated(t)}handleInput(t){this.user=t.target.value}}const Xe=E`
  :host {
    display: flex;
    flex-direction: column;
    align-items: center;
...
`;class _e extends Ze{render(){return d`
    ...
    `}}c(_e,"styles",[Xe]);window.customElements.define("login-panel",_e);class Y extends f{static get properties(){return{difficulty:{type:Number}}}constructor(){super(),this.difficulty=1}handleChangeDifficulty(e){return this.difficulty=e,this.dispatchEvent(new CustomEvent("difficulty-panel:changed",{detail:this.difficulty}))}}c(Y,"DIFFICULTY_LEVELS",Object.freeze(["Fácil","Media","Difícil","Experto"]));const Qe=E`
  :host {
    display: flex;
    flex-direction: column;
    justify-content: center;
