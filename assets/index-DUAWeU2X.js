...
`;class si extends Cn{render(){return w` <div class="score">${this._points}</div>`}}_(si,"styles",[Rn]);window.customElements.define("score-panel",si);class Pn extends M{constructor(){super();_(this,"handleLogin",()=>this.dispatchEvent(new CustomEvent("login-panel:login",{detail:this.user})))}static get properties(){return{user:{type:String}}}updated(e){super.updated(e)}handleInput(e){this.user=e.target.value}}const Tn=se`
  :host {
    display: flex;
    flex-direction: column;
    align-items: center;
